...
import type {
  ...
  TaskBoardItem,
  TaskBoardNode,
} from "../_types/board.types";
import { buildInitialGroups } from "../_lib/mock_data";
import {
  DEFAULT_LABEL_DEFS,
  DEFAULT_STATUS_DEFS,
  DEFAULT_TAG_DEFS,
...
function initialState(): TaskBoardState {
  return {
...
export function useTaskBoard() {
  ...
  const commitEditName = useCallback(() => {
    setState((s) => {
      ...
      return { ...s, editing_id: null, groups: updateNodeById<TaskBoardNode>(s.groups, s.editing_id, (n) => ({ ...n, name })) };
    });
  }, []);
  ...
  const setCellValue = useCallback((node_id: string, column_id: string, value: CellValue) => {
    setState((s) => ({
      ...s,
      groups: updateNodeById<TaskBoardNode>(s.groups, node_id, (n) => ({ ...n, values: { ...n.values, [column_id]: value } })),
    }));
  }, []);
  ...
  const toggleArrayValue = useCallback((node_id: string, column_id: string, option: string) => {
    setState((s) => ({
      ...s,
      groups: updateNodeById<TaskBoardNode>(s.groups, node_id, (n) => {
  ...
  const clearCellValue = useCallback((node_id: string, column_id: string) => {
    setState((s) => ({
      ...s,
      groups: updateNodeById<TaskBoardNode>(s.groups, node_id, (n) => ({ ...n, values: { ...n.values, [column_id]: undefined } })),
      open_cell_menu_key: null,
    }));
  }, []);
  ...
  const duplicateNode = useCallback(
    (id: string, with_subs: boolean) => {
      setState((s) => {
        ...
        if (!original) return s;
        const copy: TaskBoardNode = { ...original, id: nextId("copy"), name: `${original.name} (copy)`, values: { ...original.values } };
  ...
  const convertSubToItem = useCallback((sub_id: string) => {
    setState((s) => {
      ...
      const promoted: TaskBoardItem = { ...removed_sub, subs: [] };
  ...
  const convertItemToSub = useCallback((item_id: string, target_item_id: string) => {
    setState((s) => {
      const { groups: without, removed_item } = removeNodeById(s.groups, item_id);
      if (!removed_item) return s;
      const demoted: TaskBoardNode = { id: removed_item.id, name: removed_item.name, values: removed_item.values };
  ...
  const addGroup = useCallback(() => {
    setState((s) => {
      ...
      const new_group: TaskBoardGroup = {
        key,
        title: "New group",
        color,
        tint: color,
  ...
  const addColumn = useCallback(
    (group_key: string, scope: ColumnScope, kind: ColumnKind, label: string, default_width: number) => {
      setState((s) => {
        ...
        return {
          ...s,
          groups: s.groups.map((g) => (g.key !== group_key ? g : { ...g, [list_key]: [...(g[list_key as keyof TaskBoardGroup] as ColumnDef[]), column] })),
          open_picker_key: null,
        };
      });
    },
    ...
    setState((s) => {
      ...
      return {
        ...
        groups: s.groups.map((g) => {
          ...
          const in_base = (g[base_key as keyof TaskBoardGroup] as ColumnDef[]).some((c) => c.id === column_id);
          if (in_base) {
            return { ...g, [base_key]: (g[base_key as keyof TaskBoardGroup] as ColumnDef[]).map((c) => (c.id === column_id ? { ...c, title } : c)) };
          }
          return { ...g, [list_key]: (g[list_key as keyof TaskBoardGroup] as ColumnDef[]).map((c) => (c.id === column_id ? { ...c, title } : c)) };
        }),
      };
    });
  }, []);
  ...
  const deleteColumn = useCallback((group_key: string, scope: ColumnScope, column_id: string) => {
    setState((s) => {
      ...
      return {
        ...s,
        groups: s.groups.map((g) => (g.key !== group_key ? g : { ...g, [list_key]: (g[list_key as keyof TaskBoardGroup] as ColumnDef[]).filter((c) => c.id !== column_id) })),
        open_column_menu_key: null,
      };
    });
  }, []);
    ...
    (group_key: string, scope: ColumnScope, column_id: string) => {
      setState((s) => {
        ...
        return {
          ...
          groups: s.groups.map((g) => {
            if (g.key !== group_key) return g;
            const list = g[list_key as keyof TaskBoardGroup] as ColumnDef[];
  ...
  const copyRowLink = useCallback((id: string) => {
    ...
    if (typeof navigator !== "undefined" && navigator.clipboard) {
      navigator.clipboard.writeText(`${typeof window !== "undefined" ? window.location.origin : ""}/test/task-board#${id}`).catch(() => {});
    }
  }, []);
...
export type TaskBoardActions = ReturnType<typeof useTaskBoard>["actions"];
