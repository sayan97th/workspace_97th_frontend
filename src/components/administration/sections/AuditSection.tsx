...
const AuditSection: React.FC<AuditSectionProps> = ({ admin }) => (
  <div>
    ...
    <div className="grid grid-cols-[120px_minmax(120px,1fr)_130px_minmax(150px,1.4fr)_120px_90px_80px] gap-3 px-2.5 pb-2.5 text-[11px] font-bold uppercase tracking-[0.03em] text-[#7e8889]">
      <span>Timestamp</span>
      <span>User</span>
      <span>Event</span>
    ...
    {admin.audit_rows.map((row) => {
      ...
      return (
        <div
          key={row.id}
          className="grid grid-cols-[120px_minmax(120px,1fr)_130px_minmax(150px,1.4fr)_120px_90px_80px] items-center gap-3 border-b border-white/[0.045] px-2.5 py-[11px] text-[12.5px] text-[#d7dcdc]"
        >
          <span>{row.timestamp}</span>
          <span className="truncate font-semibold text-[#edf1f1]">{user?.name ?? "Unknown"}</span>
          <span>{row.event}</span>
          <span className="truncate text-[#9aa4a5]">{row.description}</span>
          <span className="text-[#9aa4a5]">{row.ip}</span>
          <span className="text-[#9aa4a5]">{row.browser}</span>
          <span className="text-[#9aa4a5]">{row.os}</span>
        </div>
      );
    })}
  </div>
