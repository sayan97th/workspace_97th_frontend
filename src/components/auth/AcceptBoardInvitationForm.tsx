...
import type { BoardInvitationPreview } from "@/types/board-invitation";
...
type PreviewState =
  | { status: "loading" }
  | { status: "not_found" }
  | { status: "loaded"; preview: BoardInvitationPreview };

/**
 * Accept/decline screen for the emailed "Invite to this board" link
 * (`/board-invitations/{code}`). Structurally a copy of
 * `AcceptInvitationForm` (the workspace-wide equivalent), trimmed to a
 * single implicit "viewer" grant, and redirecting straight into the board
 * itself once accepted rather than the workspace root.
 */
export default function AcceptBoardInvitationForm({ code }: Props) {
  const router = useRouter();
  const { acceptBoardInvitation } = useAuth();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (preview_state.preview.account_exists) {
        await acceptBoardInvitation(code, { password });
      } else {
        await acceptBoardInvitation(code, {
          first_name,
          last_name,
          password,
      ...
      router.push(getValidRedirectUrl(`/boards/${preview_state.preview.board.id}`));
    } catch (err: unknown) {
  ...
  const handleDecline = async () => {
    setIsDeclining(true);
    try {
      await boardInvitationService.declineInvitation(code);
      setIsDeclined(true);
    } catch {
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-shell-border dark:bg-shell-panel sm:p-8">
          ...
          {preview_state.status === "loaded" && preview_state.preview.status === "expired" && (
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                This invitation to view {preview_state.preview.board.label} has expired. Ask the
          ...
          {preview_state.status === "loaded" && is_declined && (
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                You won&apos;t be added to {preview_state.preview.board.label}.
              </p>
            </div>
          )}
                ...
                <div className="mb-6 flex items-center gap-3 sm:mb-8">
                  ...
                  <div>
                    <h1 className="font-semibold text-gray-800 text-title-sm dark:text-white/90">
                      View {preview_state.preview.board.label}
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {preview_state.preview.inviter_name} invited you to view this board in{" "}
                      <span className="font-medium text-gray-700 dark:text-shell-text-secondary">
                        {preview_state.preview.workspace.name}
                      </span>
                      .
                    </p>
                  </div>
