...
const ROW_DISABLED = "flex h-[34px] w-full items-center gap-2.5 rounded-[6px] px-2 text-left text-[13px] text-[#b3b8ca] cursor-default";
...
export default function ColumnMenu({
  ...
  return (
    <PopoverPanel onClose={onClose} className="left-1/2 top-full w-[268px] -translate-x-1/2 p-2.5">
      ...
      <div className="flex flex-col gap-0.5 pt-1.5" onMouseLeave={() => openSub(null)}>
        {column && (
          <div className="relative" onMouseEnter={() => openSub("settings")}>
            <div className={`${ROW_ITEM} cursor-pointer`} style={{ background: sub === "settings" ? "#f1f3f9" : "transparent" }}>
              <span className="w-4 text-[#6b7189]">
        ...
        {column && (
          <button type="button" onMouseEnter={() => openSub(null)} onClick={() => { onRequestFilter(); onClose(); }} className={ROW_ITEM}>
            <span className="w-4 text-[#6b7189]">
        ...
        <button type="button" onMouseEnter={() => openSub(null)} onClick={() => onSort(sort_dir === "asc" ? null : "asc")} className={ROW_ITEM}>
          <span className="w-4 text-[#6b7189]">↑</span>
          <span className="flex-1">{sort_dir === "asc" ? "Remove ascending sort" : "Sort ascending"}</span>
        </button>
        <button type="button" onMouseEnter={() => openSub(null)} onClick={() => onSort(sort_dir === "desc" ? null : "desc")} className={ROW_ITEM}>
          <span className="w-4 text-[#6b7189]">↓</span>
          <span className="flex-1">{sort_dir === "desc" ? "Remove descending sort" : "Sort descending"}</span>
        </button>

        <button type="button" onMouseEnter={() => openSub(null)} onClick={() => { onCollapseAll(); onClose(); }} className={ROW_ITEM}>
          <span className="w-4 text-[#6b7189]">
        ...
        {column && (
          is_group_by_eligible ? (
            <button type="button" onMouseEnter={() => openSub(null)} onClick={() => { onRequestGroupBy(); onClose(); }} className={ROW_ITEM}>
              <span className="w-4 text-[#6b7189]">
        ...
        {column && (
          <>
            ...
            <button type="button" onMouseEnter={() => openSub(null)} onClick={() => { onDuplicate(); onClose(); }} className={ROW_ITEM}>
              <span className="w-4 text-[#6b7189]">
            ...
            <div className="relative" onMouseEnter={() => openSub("add")}>
              <div className={`${ROW_ITEM} cursor-pointer`} style={{ background: sub === "add" ? "#f1f3f9" : "transparent" }}>
                <span className="w-4 text-[#6b7189]">
              ...
              {sub === "add" && (
                <div className="absolute left-[266px] top-[-6px] z-10">
                  <ColumnPicker query={flyout_query} onQueryChange={setFlyoutQuery} onPick={handlePickAddRight} onClose={() => openSub(null)} align="left" />
                </div>
              )}
            </div>

            <div className="relative" onMouseEnter={() => openSub("type")}>
              <div className={`${ROW_ITEM} cursor-pointer`} style={{ background: sub === "type" ? "#f1f3f9" : "transparent" }}>
                <span className="w-4 text-[#6b7189]">
        ...
        {column && can_delete && (
          <>
            ...
            <button
              ...
              className="flex h-[34px] w-full items-center gap-2.5 rounded-[6px] px-2 text-left text-[13px] text-[#b02f43] hover:bg-[#fdf2f4]"
            >
