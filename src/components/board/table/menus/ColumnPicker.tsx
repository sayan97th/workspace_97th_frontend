  ...
  return (
    <PopoverPanel onClose={onClose} className={`top-full w-[360px] p-4 ${align === "right" ? "right-0" : "left-0"}`}>
      <div className="flex items-center gap-2.5">
        <div className="flex h-9 flex-1 items-center gap-2 rounded-[8px] border-[1.5px] border-boardtree-accent px-2.5">
          <svg viewBox="0 0 14 14" width="13" height="13" className="text-boardtree-text-muted"><circle cx="6" cy="6" r="4.2" fill="none" stroke="currentColor" strokeWidth="1.4" /><path d="M9.2 9.2 L12.4 12.4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" /></svg>
          <input
            autoFocus
            value={query}
            onChange={(e) => onQueryChange(e.target.value)}
            placeholder="Search or describe your column"
            className="flex-1 bg-transparent text-[13px] text-boardtree-text outline-none"
          />
        </div>
        <button type="button" onClick={onClose} className="flex h-[30px] w-[30px] items-center justify-center rounded-[6px] text-boardtree-text-muted hover:bg-boardtree-hover">
      ...
      {SECTIONS.map((section) => {
        ...
        return (
          <div key={section} className="pt-3">
            <div className="pb-2 text-[11.5px] text-boardtree-text-muted">{section}</div>
            <div className="grid grid-cols-2 gap-[2px_4px]">
              {items.map((type) => (
                <button
                  ...
                  className="flex items-center gap-2.5 rounded-[7px] px-2 py-1.5 hover:bg-boardtree-hover"
                >
                  ...
                  <span className="text-[13.5px] text-boardtree-text">{type.label}</span>
                </button>
              ))}
            </div>
          </div>
