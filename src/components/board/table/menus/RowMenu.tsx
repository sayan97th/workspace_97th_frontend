...
const CHEVRON_ICON = (
  <svg viewBox="0 0 12 12" width="10" height="10"><path d="M4.5 3 L8 6 L4.5 9" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
);

/**
 * Per-row "..." menu for a board table item/subitem. Anchors a {@link BoardPopover} to the
 * trigger button (instead of the old absolute-positioned panel that flowed in-document and,
 * for the last row of a table, spilled past the scroll container's bottom edge and grew the
 * scrollable area while open — the cause of the scroll jitter this replaces). Nested submenus
 * (Move to / Duplicate / Convert to subitem) use {@link MenuFlyout} and open on click rather
 * than hover, the same convention as OverflowControl's "Item height" submenu, so there's no
...
export default function RowMenu({
  ...
  return (
    <>
      <BoardPopover anchor_el={anchor_el} is_open onClose={onClose} width={244} align="start">
        <div className="flex flex-col p-1.5">
          <button type="button" onClick={() => { onOpen(); onClose(); }} className={ROW_ITEM}>
          ...
          <button type="button" onClick={() => { onCopyLink(); }} className={ROW_ITEM}>
            <span className="flex w-4 items-center justify-center text-boardtree-text-muted">
          ...
          {move_targets.length > 0 && (
            <button
              ...
              style={{ background: open_submenu === "move" ? "var(--color-boardtree-hover)" : "transparent" }}
            >
              <span className="flex w-4 items-center justify-center text-boardtree-text-muted">
              ...
              <span className="flex-1 text-left">{is_sub ? "Move to item" : "Move to group"}</span>
          ...
          <button
            type="button"
            ref={dup_row_ref}
            onClick={() => toggleSubmenu("dup")}
            className={SUBMENU_TRIGGER}
            style={{ background: open_submenu === "dup" ? "var(--color-boardtree-hover)" : "transparent" }}
          >
            <span className="flex w-4 items-center justify-center text-boardtree-text-muted">
          ...
          <button type="button" onClick={() => { onCreateBelow(); onClose(); }} className={ROW_ITEM}>
            <span className="flex w-4 items-center justify-center text-boardtree-text-muted">
          ...
          {!is_sub && (
            <button type="button" onClick={() => { onAddSubitem(); onClose(); }} className={ROW_ITEM}>
              <span className="flex w-4 items-center justify-center text-boardtree-text-muted">
          ...
          <button type="button" onClick={() => { onArchive(); onClose(); }} className={ROW_ITEM}>
            <span className="flex w-4 items-center justify-center text-boardtree-text-muted">
          ...
          <button type="button" onClick={() => { onDelete(); onClose(); }} className="flex h-[34px] w-full items-center gap-2.5 rounded-[6px] px-2 text-[13px] text-boardtree-danger hover:bg-boardtree-danger-hover">
      ...
      {move_targets.length > 0 && (
        <MenuFlyout anchor_el={move_row_ref.current} is_open={open_submenu === "move"} onClose={closeSubmenu} side="right" width={222}>
          <div className="max-h-[300px] overflow-y-auto p-1.5">
            {move_targets.map((t) => (
              <button
                ...
                key={t.id}
                disabled={t.current}
                onClick={() => { if (!t.current) { onMoveTo(t.id); closeSubmenu(); onClose(); } }}
      ...
      <MenuFlyout anchor_el={dup_row_ref.current} is_open={open_submenu === "dup"} onClose={closeSubmenu} side="right" width={232}>
        <div className="p-1.5">
          <button type="button" onClick={() => { onDuplicate(false); closeSubmenu(); onClose(); }} className="flex h-8 w-full items-center rounded-[6px] px-2.5 text-left text-[13px] text-boardtree-text hover:bg-boardtree-hover">
            {is_sub ? "This subitem" : "This item without subitems"}
          </button>
          {!is_sub && (
            <button type="button" onClick={() => { onDuplicate(true); closeSubmenu(); onClose(); }} className="flex h-8 w-full items-center rounded-[6px] px-2.5 text-left text-[13px] text-boardtree-text hover:bg-boardtree-hover">
          ...
          <div className="max-h-[260px] overflow-y-auto p-1.5">
            ...
            {convert_targets.map((t) => (
              <button
                type="button"
                key={t.id}
                onClick={() => { onConvertToSubOf(t.id); closeSubmenu(); onClose(); }}
