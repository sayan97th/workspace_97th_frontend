    ...
    <PopoverPanel onClose={onClose} className="w-[244px] p-1.5" style={panel_style}>
      <div className="flex flex-col" onMouseLeave={() => setSub(null)}>
        ...
        <button type="button" onMouseEnter={() => setSub(null)} onClick={() => { onCopyLink(); }} className={ROW_ITEM}>
          ...
          {copied && <span className="text-[11.5px] text-[#4f6bed]">copied</span>}
        </button>
        ...
        {move_targets.length > 0 && (
          <div className="relative" onMouseEnter={() => setSub("move")}>
            ...
            {sub === "move" && (
              <div className="absolute left-[238px] top-[-6px] z-10 w-[222px] rounded-[10px] border border-[#e3e6ef] bg-white p-1.5 shadow-[0_16px_44px_rgba(30,34,55,0.22)]">
                {move_targets.map((t) => (
                  <button
                    type="button"
                    key={t.id}
                    ...
                    className="flex h-8 w-full items-center gap-2.5 rounded-[6px] px-2.5 text-left text-[13px] hover:bg-[#f1f3f9] disabled:cursor-default"
                    style={{ color: t.current ? "#8b90a6" : "#262b45" }}
                  >
                    <span className="flex-1 truncate">{t.label}</span>
                    {t.current && <span className="text-[12px] text-[#4f6bed]">✓</span>}
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="relative" onMouseEnter={() => setSub("dup")}>
          ...
          {sub === "dup" && (
            <div className="absolute left-[238px] top-[-6px] z-10 w-[232px] rounded-[10px] border border-[#e3e6ef] bg-white p-1.5 shadow-[0_16px_44px_rgba(30,34,55,0.22)]">
              <button type="button" onClick={() => { onDuplicate(false); onClose(); }} className="flex h-8 w-full items-center rounded-[6px] px-2.5 text-left text-[13px] text-[#262b45] hover:bg-[#f1f3f9]">
                {is_sub ? "This subitem" : "This item without subitems"}
              </button>
              {!is_sub && (
                <button type="button" onClick={() => { onDuplicate(true); onClose(); }} className="flex h-8 w-full items-center rounded-[6px] px-2.5 text-left text-[13px] text-[#262b45] hover:bg-[#f1f3f9]">
        ...
        <div className="relative" onMouseEnter={() => setSub("convert")}>
          <div
            ...
            style={{ background: sub === "convert" ? "#f1f3f9" : "transparent" }}
          ...
          {sub === "convert" && !is_sub && convert_targets.length > 0 && (
            <div className="absolute left-[238px] top-[-6px] z-10 max-h-[260px] w-[238px] overflow-y-auto rounded-[10px] border border-[#e3e6ef] bg-white p-1.5 shadow-[0_16px_44px_rgba(30,34,55,0.22)]">
              <div className="px-2.5 pb-2 pt-1.5 text-[12px] text-[#6b7189]">Make it a subitem of</div>
              {convert_targets.map((t) => (
                <button
                  type="button"
                  key={t.id}
                  onClick={() => { onConvertToSubOf(t.id); onClose(); }}
                  className="flex h-8 w-full items-center truncate rounded-[6px] px-2.5 text-left text-[13px] text-[#262b45] hover:bg-[#f1f3f9]"
                >
                  {t.label}
                </button>
              ))}
