...
const TOOL_BUTTON = "flex items-center gap-1.5 rounded-[6px] px-2.5 py-[7px] text-[13.5px] text-boardtree-text-secondary hover:bg-boardtree-hover";

export default function TableToolbar({ summary_text, onNewItem }: TableToolbarProps) {
  return (
    <div className="flex items-center gap-2 border-b border-boardtree-border-soft bg-boardtree-surface px-7 py-3.5">
      <button type="button" onClick={onNewItem} className="flex items-center rounded-[6px] bg-boardtree-accent px-3.5 py-2 text-[13.5px] font-medium text-white hover:bg-boardtree-accent-hover">
        New item
      </button>
