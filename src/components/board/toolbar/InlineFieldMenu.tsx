  ...
  return (
    <div ref={container_ref} className={`relative min-w-0 ${className}`} style={width ? { width } : { flex: 1 }}>
      <button
        ...
        className={`flex h-[38px] w-full items-center justify-between gap-2 rounded-lg border bg-white/[0.04] px-3 text-[13.5px] transition-colors ${
          is_open ? "border-brand-500" : "border-white/[0.12] hover:border-white/[0.28]"
        }`}
      >
        <span className="flex min-w-0 flex-1 items-center gap-[9px] overflow-hidden">{renderValue()}</span>
        <ChevronDownIcon
          size={11}
          className={`flex-none text-[#8a9495] transition-transform ${is_open ? "rotate-180" : ""}`}
        />
      </button>

      {is_open && (
        <div
          className="absolute left-0 top-[calc(100%+5px)] z-[210] w-full min-w-[220px] overflow-y-auto rounded-[9px] border border-white/[0.12] bg-[#182927] p-1.5 shadow-2xl shadow-black/50"
          style={{ maxHeight: menu_max_height }}
        >
          {menu_heading && (
            <div className="px-2 pb-1 pt-1 text-[11.5px] font-semibold tracking-wide text-[#77827f]">
              {menu_heading}
            </div>
          )}
          {options.map((option) => {
            ...
            return (
              <div
                ...
                className={`flex cursor-pointer items-center gap-[10px] rounded-md px-2 py-2 text-[13.5px] font-medium text-[#e4e9e9] hover:bg-white/[0.09] ${
