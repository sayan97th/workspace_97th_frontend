...
const ProfileModal: React.FC<ProfileModalProps> = ({ is_open, onClose, onOpenTeams }) => {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="My Profile" className="fixed inset-0 z-[400] flex items-center justify-center p-6">
      ...
      <div className="relative z-[401] flex h-[780px] max-h-[92vh] w-[1180px] max-w-full overflow-hidden rounded-[18px] border border-white/[0.08] bg-[#0f1c1c] font-outfit text-[#e9eded] shadow-[0_30px_70px_rgba(0,0,0,0.55)]">
        <button
          type="button"
          onClick={onClose}
          aria-label="Close"
