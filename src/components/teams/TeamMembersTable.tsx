...
const TeamMembersTable: React.FC<TeamMembersTableProps> = ({
  ...
  <div>
    <div className="flex items-center px-[10px] py-2 text-[11.5px] font-semibold tracking-[0.04em] text-[#6e7b7d]">
    ...
    {members.map((member) => (
      <div
        key={member.id}
        className="flex items-center rounded-[9px] px-[10px] py-[9px] transition-colors hover:bg-white/[0.05]"
      >
        <span className="flex w-[230px] min-w-0 flex-none items-center gap-[9px]">
          <PersonAvatar person={member} size={28} />
          <span className="truncate text-[13.5px] font-medium text-[#e9eded]">{member.name}</span>
          {member.is_owner ? (
        ...
        <span className="min-w-0 flex-1 truncate text-[13px] text-[#9aa4a5]">{member.email}</span>
        <span className="w-[200px] flex-none truncate text-[13px] text-[#9aa4a5]">
          {member.title ?? "—"}
        </span>
      </div>
    ))}
    ...
    {members.length === 0 ? (
      <div className="px-[10px] py-10 text-center text-[13px] text-[#7e8889]">{empty_label}</div>
    ) : null}
  </div>
);
