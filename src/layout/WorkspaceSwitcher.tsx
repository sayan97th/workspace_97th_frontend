...
const WorkspaceRow: React.FC<WorkspaceRowProps> = ({ workspace, is_active, onSelect }) => (
  <button
    ...
    className={`flex w-full items-center gap-[11px] rounded-[9px] px-2.5 py-2.5 text-left transition-colors hover:bg-white/[0.06] ${
      is_active ? "bg-white/[0.06]" : ""
    }`}
  >
...
const SectionLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="px-1.5 pb-1.5 pt-3.5 font-mono-accent text-[11px] uppercase tracking-[0.05em] text-gray-400">
    {children}
  </div>
);
    ...
    <div className="relative">
      <div className="flex gap-2">
        <button
          ...
          className={`flex flex-1 items-center gap-2.5 rounded-[10px] border bg-gray-800 px-3 py-2.5 text-left transition-colors hover:border-brand-500/60 ${
            is_open ? "border-brand-500/60" : "border-white/10"
          }`}
        >
          <WorkspaceBadge workspace={active_workspace} size={24} notchColor="#050d0d" />
          <span className="flex-1 truncate text-sm font-semibold text-gray-50">
            {active_workspace.name}
          </span>
          <ChevronDownIcon
            size={12}
            className={`flex-none text-gray-400 transition-transform duration-150 ${
              is_open ? "rotate-180" : ""
            }`}
          />
        </button>
      ...
      {is_open && (
        <>
          ...
          <div
            className="absolute left-0 right-0 top-[calc(100%+8px)] z-[120] rounded-xl border border-white/[0.12] bg-[#0f1c1c] p-2.5 shadow-2xl"
            role="listbox"
          >
            <div className="flex items-center gap-2.5 rounded-[9px] border border-white/10 bg-gray-700 px-3 py-2.5">
              ...
              <input
                ...
                autoFocus
                className="min-w-0 flex-1 bg-transparent text-sm text-gray-100 placeholder:text-gray-400 focus:outline-none"
              />
            </div>
            ...
            {has_no_results && (
              <div className="px-2.5 py-6 text-center text-sm text-gray-400">
                No workspaces match &ldquo;{query.trim()}&rdquo;
              </div>
            )}

            <div className="mt-2.5 flex items-center gap-2 border-t border-white/[0.08] pt-2.5">
              <button
                type="button"
                onClick={handleAddWorkspace}
                className="flex flex-1 items-center justify-center gap-2 rounded-[9px] px-2.5 py-2.5 text-sm font-medium text-gray-200 transition-colors hover:bg-white/[0.06]"
              >
                <PlusIcon size={14} />
                Add workspace
              </button>
